...
export default function TodaySummaryModal({ isOpen, onClose }: TodaySummaryModalProps) {
  ...
  const fetchTodayReservations = async () => {
    ...
    try {
      ...
      const todayReservations = allReservations
        .filter((r: Reservation) => r.date === today && r.status === 'reserved')
        .sort((a: Reservation, b: Reservation) => a.time.localeCompare(b.time));
    ...
    >
      ...
      >
        <div className="flex items-center justify-between mb-6">
          <h2 className="text-3xl font-bold text-white">{getTodayDate()}</h2>
          <button
            onClick={onClose}
